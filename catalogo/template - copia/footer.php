...
<?php }?>
  ...
  <?php  
if(CARRITO==1)	{
	$m = $mysqli->query("SELECT SUM(cantidad) cantidad,SUM(precio*cantidad) total FROM car_temp_pedido WHERE token='".$_SESSION['sesiones'][0]."' AND cliente='".NUM_CLIENTE."'");
	$rox = $m->fetch_assoc();
	$total=$rox['total'];
	if ($rox['total']>0){ ?>
...
<?php } if(CARRITO==0 || $total==0){?>
	...
	<a href="https://menucatalogo.com" style="text-decoration: none; vertical-align: middle;" target="_blank"><font color="#FFFFFF"; size="3"> By</font>
     <?php $logo=ID_CONSENTRADOR; if ($logo > 0){ ?>
		<img src="<?php echo URL_BASE ?>img/concentradores/<?php echo ID_CONSENTRADOR ?>.png" height="50"></a> 
    <?php }else{?>
		<img src="<?php echo URL_BASE ?>img/logo_qr/logoLogin.png" width="120"></a>
    <?php }?>
