...
//print_r($_SESSION);
//exit();
if($_POST['mesa']>0) $confirmar=1; else $confirmar=0;
require("../conexion/conexion.php");
$m=$mysqli->query("SELECT idPedido FROM car_pedido WHERE token='".$_SESSION['sesiones'][0]."' AND idComprador='".$_SESSION['sesiones'][1]."'");
$id = $m->fetch_assoc();
if($id['idPedido']==''){
		$inser = "INSERT INTO car_pedido (envio,idCliente,idComprador,token,mesa,fechaPedido,total,tipoPago,vendedor,estadoPedido) 
	...
	$sql = "UPDATE car_pedido SET fechaPedido='".date('Y-m-d H:i')."',total='".$_SESSION['sesiones'][3]."',mesa='".$_SESSION['sesiones'][4]."',tipoPago='',vendedor='".$_GET['vendedor']."',estadoPedido='PEDIDO RECIBIDO' WHERE idPedido=".$rox['idPedido']."";
	$mysqli->query($sql);
}
//exit();
$m=$mysqli->query("SELECT * FROM car_temp_pedido WHERE token='".$_SESSION['sesiones'][0]."' AND cliente='".$_GET['idCliente']."'");
	while($row = $m->fetch_assoc()){
		$inser = "INSERT INTO car_tabla_pedido (cliente,token,categoria,idProducto,producto,cantidad,nota,precio,confirmar) 
		VALUES ('".$row['cliente']."','".$row['token']."','".$row['categoria']."','".$row['idProducto']."','".$row['producto']."','".$row['cantidad']."','".$row['nota']."','".$row['precio']."',".$confirmar.")"; 
		$mysqli->query($inser);
