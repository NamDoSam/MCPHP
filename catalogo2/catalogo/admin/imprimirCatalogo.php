...
$sql = $mysqli->query('SELECT * FROM clientes WHERE slug="'.$_GET['slug'].'"');
$cli = $sql->fetch_assoc();
?>
<!DOCTYPE html>
<html lang="es">
...
<body>
<h2 align="center">- <?php echo $cli['razonSocial'] ?> -</h2>
<?php
$query_cities = $mysqli->query('SELECT * FROM rubros WHERE idCliente="'.$cli['idCliente'].'" AND ocultar=0');
...
WHERE ca.idCliente="'.$cli['idCliente'].'" AND ca.idItem="'.$row['idItem'].'" AND ca.ocultar=0 AND ca.estado=0 
