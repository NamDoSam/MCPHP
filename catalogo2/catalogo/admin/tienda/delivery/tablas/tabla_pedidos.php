 ...
 <input type="hidden" name="idPedido" value="<?php echo $_GET['idPedido'] ?>">
    <table width="580" border="1" bordercolor="#CCCCCC" align="center" cellpadding="5" cellspacing="0">
      <thead>
        <tr>
          <th><div align="center"><?php echo TEXTO_16 ?></div></th>
          <th><?php echo TABLA_12 ?></th>
          <th><?php echo TABLA_7 ?></th>
          <th><div align="center"><?php echo TEXTO_17 ?></div></th>
        </tr>
      </thead>
      <tbody>
<?php 
...
alt="" width="60" height="" /></td>
    ...
    <i class="fa fa-print fa-2x"></i> <?php echo BOTON_12 ?></a></div>
  </form>
</div>
...
<div id="popup" class="popup">
  ...
  <br>
  <h2>&nbsp;&nbsp;&nbsp;<?php echo TEXTO_18." N°".$_GET['idPedido'] ?>&nbsp;&nbsp;&nbsp;</h2>
  <hr>
<div align="center">
<form name="form2" method="post" action="shopcar.php?slug=<?php echo $_GET['slug'] ?>&fecha1=<?php echo $_GET['fecha1'] ?>&fecha2=<?php echo $_GET['fecha2'] ?>">
 ...
 <input type="hidden" name="celComp" value="<?php echo $celComp ?>">
  <select name="estadoPedido_es"  class="form-control" onchange="this.form.submit()">
	<?php $est=$mysqli->query("SELECT * FROM estado_pedido ORDER BY idEstado");
	while($data = $est->fetch_assoc()){?>  
    <option value="<?php echo $data['estadoPedido_es'] ?>"<?php if($data['estadoPedido_es']==$_GET['estadoPedido_es']) echo "selected"; ?> ><?php echo $data['estadoPedido_es'] ?></option>
    <?php }?>
  </select>
  <br><br>
